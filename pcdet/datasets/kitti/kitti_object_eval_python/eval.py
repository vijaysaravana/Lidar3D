...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        # elif (current_cls_name == "Car".lower()):
            valid_class = 0
        else:
            valid_class = -1
        ignore = False
...
def compute_statistics_jit(overlaps,
    ...
    # print(f'thresholds in jit: {thresholds}')
    thresh_idx = 0
    ...
    for i in range(gt_size):
        ...
        # print(f'Valid detection 229 :: {valid_detection}')
        if (valid_detection == NO_DETECTION) and ignored_gt[i] == 0:
            fn += 1
            # print(f'Valid detection 232 FN :: {fn}')
        ...
        # elif valid_detection != NO_DETECTION:
            ...
            tp += 1
            # print(f'Valid detection 238 TP :: {tp}')
    ...
    if compute_fp:
        ...
        if compute_aos:
            ...
            if tp > 0 or fp > 0:
                ...
                similarity = -1
    return tp, fp, fn, similarity, thresholds[:thresh_idx], thresh_idx
...
def eval_class(gt_annos,
    ...
    """
    print(f'Len gt :: {len(gt_annos)} Len dt_annos :: {len(dt_annos)}')
    if(len(gt_annos) != len(dt_annos)):
        gt_annos = gt_annos[:-1 * (len(gt_annos) - len(dt_annos))]
        print(f'Changing Len gt :: {len(gt_annos)} Len dt_annos :: {len(dt_annos)}')
    
    assert len(gt_annos) == len(dt_annos)

    # print(f'Eval det_annos :: {dt_annos}')
    # print(f'Kitti infos annos :: {gt_annos}')
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                thresholdss = []
                for i in range(len(gt_annos)):
                    # print(f'GT size :: {gt_datas_list[i].shape[0]}')
                    rets = compute_statistics_jit(
                        ...
                        thresh=0.0,
                        compute_fp=False)
                    tp, fp, fn, similarity, thresholds, thresh_idx = rets
                    # print(f'thresholds len :: {len(thresholds)} thresholds : {thresholds}')
                    # print(f'Thresh index :: {thresh_idx} - thresholds :: {thresholds.shape}')
                    thresholdss += thresholds.tolist()
                    # thresholdss += thresholds
                thresholdss = np.array(thresholdss)
                thresholds = get_thresholds(thresholdss, total_num_valid_gt)
                thresholds = np.array(thresholds)
                # print(f'Thresholds : {thresholdss}')
